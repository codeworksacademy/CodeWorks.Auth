...
builder.Services.AddAuthModule<AppUser, AppUserStore>(
  ...
  permissionPolicies: ["CanReadReports", "CanManageUsers"],
  configurePasskeyOptions: passkey =>
  {
    passkey.RpId = "localhost";
    passkey.RpName = "Local App";
    passkey.ExpectedOrigin = "https://localhost:5001";
  });

builder.Services.AddAuthDatabaseStores(async ct =>
{
  DbConnection connection = new SqlConnection(builder.Configuration.GetConnectionString("AuthDb"));
